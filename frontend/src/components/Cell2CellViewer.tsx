...
export const Cell2CellViewer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const processData = (dataArray: CellData[]) => {
      return d3.rollup(
        dataArray,
        (v) => {
    ...
    const calculateMaxStack = (data: Map<string, { [key: string]: number }>) => {
    ...
    // Color scale
    const color = d3.scaleOrdinal<string>().domain(regions).range(d3.schemeTableau10);
    ...
    svg
      ...
      .call(d3.axisLeft(y).tickFormat((d) => Math.abs(d).toString())); // Show absolute values
